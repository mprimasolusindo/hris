...
export default function Index({
    ...
    return (
        <MasterCrudPage<ComponentRow>
            ...
            initialForm={{ name: '', is_taxable: true }}
            mapRowToForm={(row) => ({
                name: row.name,
                is_taxable: row.is_taxable,
            })}
            storeUrl={route('master.allowance-types.store')}
            updateUrl={(id) => route('master.allowance-types.update', id)}
            destroyUrl={(id) => route('master.allowance-types.destroy', id)}
        />
    );
}
